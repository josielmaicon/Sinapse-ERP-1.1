...
const statusColors = {
  aberto: "text-2xl bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-400",
  fechado: "text-2xl bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-400",
  pausado: "text-2xl bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-400",
};
...
export default function PosHeaderStatus({ session = mockPdvSession }) {
  ...
  return (
    <div className="w-full h-full flex items-center text-3xl justify-between font-mono p-4">
      <div className="flex items-center gap-5">
        <Badge className={cn("text-base", statusColors[session.status])}>
          {session.status.toUpperCase()}
        </Badge>
        <div className="text-2xl flex items-center gap-2 text-muted-foreground">
